...
Titration of mitotic/interphase HeLa-S3 cells: from our preprint [here](https://pubmed.ncbi.nlm.nih.gov/41394721/). H3K9ac levels were varied by mixing mitotic cells (known to have low H3K9ac) with unsynchronized/interphase cells (higher H3K9ac) in known ratios to create a standard curve. The proportions of cells used are on the left side of the figure below. On the right, a histogram of H3K9ac signal is created first for traditional read-depth normalized signal (top) and ChIP-wrangler-normalized signal (bottom). 

![H3K9ac titration dataset](GGplot_quarto_files/H3K9ac_titration_dataset_example.png) 
...
Our example dataset is generated with HOMER `annotatePeaks`, quantifying H3K9ac ChIP-seq signal at RefSeq TSSs. 

```{r}
hist_tss_hg38_tutorial_spikenorm_LP78 <- read.delim("~/Documents/Research/LP_78/hist_tss_hg38_spikenorm_LP78.txt")
```

Before cleaning, the headers are messy, and contain the original function used to make the file:
...
See full tutorial [here](https://github.com/lapatel22/tutorials/blob/main/R_tidyverse_cheatsheet.md) for tidyverse operations. We will rely on three main functions from this library for plotting: `pivot_longer`, `mutate`, and `str_match`
...
To plot our data, we simply plot our y variable (`Coverage`) vs the x variable (`Distance from the TSS`). But because the data is in tidy format, and we want to plot each sample uniquely, we must tell ggplot to group the data by `Sample`.
...
The above plot is technically showing the data we are interested in, but it's not overly informative compared to our figure. First, by setting color equal to our experimental variable (the % of interphase cells in each sample), we see the expected titration pattern has emerged. 
...
Notice as soon as a variable is mapped to an aesthetic (in this case, we set the color equal to our experiment conditions), ggplot auto-generates a legend. This is nice, but we should also make the legend more descriptive, and add more accurate plot labels in general. Plot labels are added with the `labs()` function.
...
`ChIP-wrangler` normalizes data 3 ways: to each of two spike-in species, and to the average of both (dual spike-in normalization). Currently we are plotting the results of dual spike-in normalization.

```{r}
knitr::kable(head(hist_tss_hg38_tutorial_spikenorm_LP78_long))
```

We next reformat the dataframe so that we can plot the results of normalizing to each spike-in species separately. 
...
In our example dataset, we make the linetype vary based on which species the data is normailzed to: we plot fly-normalized data with a dotted line, and yeast-normalized data with a dashed line.
...
In our example dataset, we can split our graph into facets for each technical replicate.
