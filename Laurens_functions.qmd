...
```{r, eval = FALSE}
#| label: seqstats_plots_data_prep
seqstats_tidy <- 
...
```{r, eval = FALSE}
#| label: histograms_signalarea_usage
...
#| label: scatter_plot_function_brewercolors
...
```{r, eval = FALSE}
#| label: scatter_plot_function_brewercolors_usage
...
Format: counts_p300_peaks_binned <- quantiling_peaks(counts_p300_peaks, 10)
...
We want to breakdown the ID column into components, for example the sample in row 1: 
...
NOTE: when a function contains an input variable that will be called by ggplot (for example, an x or y value) you have to use `aes_string()` instead of `aes()`. Then in the function, the input arguments need to be in string format (in quotes).

Use: 

```{r, eval = FALSE}
#| label: plot_taglength_usage
...
```{r}
#| label: base_frequency_processing
# get vector of sample tagdir names
