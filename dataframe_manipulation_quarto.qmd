...
2. Most examples will be either my own or public data commonly used in R tutorials. One example of public data is the gapminder dataset which is loaded above with `library(gapminder)`. It contains demographic/economic/health information on countries from 1952-2007. 
...
```{r} 
TRP_input_tagLen <- read.table("LP_81/LP_81/dm6_tagdirs/HelaS3_TRP_0hr_input_1_DSG-FA.dm6.GCcheck-tagdir/tagLengthDistribution.txt", sep="\t", header = T)
...
Where the # above is the column number (above changes the name of the first column)
...
Example: get rows whose column `library ID` contents matched the partial string `DPY30` OR `RBBP5`.
